...
interface PostInterface extends RepositoryInterface
{
    ...
    public function getRecentprojectsposts(int $limit = 5, int|string $categoryId = 0): Collection;

    public function getSearch(string|null $keyword, int $limit = 10, int $paginate = 10): Collection|LengthAwarePaginator;

    public function getAllprojectsposts(int $perPage = 12, bool $active = true, array $with = ['slugable']): Collection|LengthAwarePaginator;

    public function getPopularprojectsposts(int $limit, array $args = []): Collection;
