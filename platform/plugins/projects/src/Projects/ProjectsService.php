    ...
    public function handleFrontRoutes(Slug|array $slug): Slug|array|Builder
    {
        ...
        switch ($slug->reference_type) {
            case Post::class:
                ...
                $post = Post::query()
                    ->where($condition)
                    ->with(['projectscategories', 'projectstags', 'slugable', 'projectscategories.slugable', 'projectstags.slugable'])
                    ->firstOrFail();
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/projects::posts.edit_this_post'),
                        route('projectsposts.edit', $post->getKey()),
                        null,
                        'projectsposts.edit'
                    );
                }
                ...
                if (function_exists('shortcode')) {
                    shortcode()->getCompiler()->setEditLink(route('projectsposts.edit', $post->id), 'projectsposts.edit');
                }

                $category = $post->projectscategories->sortByDesc('id')->first();
                if ($category) {
                ...
                return [
                    'view' => 'post',
                    'default_view' => 'plugins/projects::themes.post',
                    'data' => compact('post'),
                    'slug' => $post->slug,
                ];
            case Category::class:
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/projects::categories.edit_this_category'),
                        route('projectscategories.edit', $category->getKey()),
                        null,
                        'projectscategories.edit'
                    );
                }

                $allRelatedCategoryIds = array_merge([$category->getKey()], $category->activeChildren->pluck('id')->all());

                $projectsposts = app(PostInterface::class)
                    ->getByCategory($allRelatedCategoryIds, (int)theme_option('number_of_projectsposts_in_a_category', 12));
                ...
                return [
                    'view' => 'category',
                    'default_view' => 'plugins/projects::themes.category',
                    'data' => compact('category', 'projectsposts'),
                    'slug' => $category->slug,
                ];
            case Tag::class:
                $stag = Tag::query()
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/projects::tags.edit_this_tag'),
                        route('projectstags.edit', $stag->getKey()),
                        null,
                        'projectstags.edit'
                    );
                }

                $projectsposts = get_projectsposts_by_tag($stag->getKey(), (int)theme_option('number_of_projectsposts_in_a_tag', 12));
                ...
                return [
                    'view' => 'stag',
                    'default_view' => 'plugins/projects::themes.stag',
                    'data' => compact('stag', 'projectsposts'),
                    'slug' => $stag->slug,
                ];
        }
