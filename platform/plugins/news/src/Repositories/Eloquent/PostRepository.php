...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('newscategories', function (Builder $query) use ($id) {
                $query->whereIn('newscategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getRelatedCategoryIds(Post|int|string $model): array
    {
        ...
        try {
            return $model->newscategories()->allRelatedIds()->toArray();
        } catch (Exception) {
            return [];
        }
    }
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('newscategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('newscategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    {
        $data = $this->model
            ->with(['slugable', 'newscategories', 'newscategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('newstags', function (Builder $query) use ($stag) {
                $query->where('newstags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentnewsposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('newscategories', function (Builder $query) use ($categoryId) {
                    $query->where('newscategories.id', $categoryId);
                });
        }
    ...
    public function getPopularnewsposts(int $limit, array $args = []): Collection
    {
        $data = $this->model
            ->with('slugable')
            ->orderByDesc('views')
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['newscategories'] !== null) {
            $newscategories = array_filter((array)$filters['newscategories']);

            $data = $data->whereHas('newscategories', function (Builder $query) use ($newscategories) {
                $query->whereIn('newscategories.id', $newscategories);
            });
        }

        if ($filters['newscategories_exclude'] !== null) {
            $data = $data
                ->whereHas('newscategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('newscategories.id', array_filter((array)$filters['newscategories_exclude']));
                });
        }
