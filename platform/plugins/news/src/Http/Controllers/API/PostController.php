...
use Botble\News\Http\Resources\ListPostResource;
use Botble\News\Http\Resources\PostResource;
use Botble\News\Models\Post;
use Botble\News\Repositories\Interfaces\PostInterface;
use Botble\News\Supports\FilterPost;
...
class PostController extends BaseController
{
    ...
    public function index(Request $request)
    {
        $data = $this->postRepository
            ->advancedGet([
                'with' => ['newstags', 'newscategories', 'author', 'slugable'],
    ...
    public function getSearch(Request $request, PostInterface $postRepository)
    {
        ...
        $newsposts = $postRepository->getSearch($query);

        $data = [
            'items' => $newsposts,
            'query' => $query,
            'count' => $newsposts->count(),
        ];
     ...
     * @queryParam after                Limit response to newsposts published after a given ISO8601 compliant date.
     * @queryParam author               Limit result set to newsposts assigned to specific authors.
     * @queryParam author_exclude       Ensure result set excludes newsposts assigned to specific authors.
     * @queryParam before               Limit response to newsposts published before a given ISO8601 compliant date.
     ...
     * @queryParam newscategories           Limit result set to all items that have the specified term assigned in the newscategories taxonomy.
     * @queryParam newscategories_exclude   Limit result set to all items except those that have the specified term assigned in the newscategories taxonomy.
     * @queryParam newstags                 Limit result set to all items that have the specified term assigned in the newstags taxonomy.
     * @queryParam newstags_exclude         Limit result set to all items except those that have the specified term assigned in the newstags taxonomy.
