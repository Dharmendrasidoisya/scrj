...
use Botble\News\Forms\CategoryForm;
use Botble\News\Http\Requests\CategoryRequest;
...
class CategoryController extends BaseController
{
    protected function breadcrumb(): Breadcrumb
    {
        return parent::breadcrumb()
            ...
            ->add(trans('plugins/news::categories.menu'), route('newscategories.index'));
    }

    public function index(Request $request)
    {
        $this->pageTitle(trans('plugins/news::categories.menu'));
    ...
    {
        $this->pageTitle(trans('plugins/news::categories.create'));
    ...
    public function store(CategoryRequest $request)
    {
        ...
        return $response
            ->setPreviousRoute('newscategories.index')
            ->setNextRoute('newscategories.edit', $category->getKey())
    ...
    {
        if (! $model) {
            $form->setUrl(route('newscategories.create'));
        }

        if (! Auth::guard()->user()->hasPermission('newscategories.create') && ! $model) {
    ...
    {
        return array_merge([
            'canCreate' => Auth::guard()->user()->hasPermission('newscategories.create'),
            'canEdit' => Auth::guard()->user()->hasPermission('newscategories.edit'),
            'canDelete' => Auth::guard()->user()->hasPermission('newscategories.destroy'),
            'createRoute' => 'newscategories.create',
            'editRoute' => 'newscategories.edit',
            'deleteRoute' => 'newscategories.destroy',
            'updateTreeRoute' => 'newscategories.update-tree',
        ], $options);
    }
}
