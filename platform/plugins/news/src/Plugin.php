<?php

namespace Botble\News;

use Botble\News\Models\Category;
use Botble\News\Models\Tag;
use Botble\Dashboard\Models\DashboardWidget;
use Botble\Menu\Models\MenuNode;
use Botble\PluginManagement\Abstracts\PluginOperationAbstract;
use Botble\Setting\Facades\Setting;
use Botble\Widget\Models\Widget;
use Illuminate\Support\Facades\Schema;

class Plugin extends PluginOperationAbstract
{
    public static function remove(): void
    {
        Schema::disableForeignKeyConstraints();
        Schema::dropIfExists('spost_newstags');
        Schema::dropIfExists('spost_newscategories');
        Schema::dropIfExists('newsposts');
        Schema::dropIfExists('newscategories');
        Schema::dropIfExists('newstags');
        Schema::dropIfExists('newsposts_translations');
        Schema::dropIfExists('newscategories_translations');
        Schema::dropIfExists('newstags_translations');

        Widget::query()
            ->where('widget_id', 'widget_newsposts_recent')
            ->each(fn (DashboardWidget $dashboardWidget) => $dashboardWidget->delete());

        MenuNode::query()
            ->whereIn('reference_type', [Category::class, Tag::class])
            ->each(fn (MenuNode $menuNode) => $menuNode->delete());

        Setting::delete([
            'news_post_schema_enabled',
            'news_post_schema_type',
        ]);
    }
}
