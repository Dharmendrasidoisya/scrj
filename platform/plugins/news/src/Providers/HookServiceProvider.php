...
use Botble\News\News\NewsService;
...
class HookServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        ...
        $this->app['events']->listen(RenderingAdminBar::class, function () {
            AdminBar::registerLink(
                trans('plugins/news::posts.post'),
                route('newsposts.create'),
                'add-new',
                'newsposts.create'
            );
        });
        ...
        if (function_exists('add_shortcode')) {
            shortcode()
                ->register(
                    $shortcodeName = 'news-newsposts',
                    trans('plugins/news::base.short_code_name'),
                    trans('plugins/news::base.short_code_description'),
                    [$this, 'renderNewsnewsposts']
                )
                ->setAdminConfig(
                    $shortcodeName,
                    function (array $attributes) {
                        ...
                        return ShortcodeForm::createFromArray($attributes)
                            ->add('paginate', 'number', [
                                'label' => trans('plugins/news::base.number_newsposts_per_page'),
                                'attr' => [
                                    'placeholder' => trans('plugins/news::base.number_newsposts_per_page'),
                                ],
                            ])
                            ->add(
                                'category_ids[]',
                                ...
                                SelectFieldOption::make()
                                    ->label(__('Select newscategories'))
                                    ->choices($newscategories)
                                    ->when(Arr::get($attributes, 'category_ids'), function (SelectFieldOption $option, $newscategoriesIds) {
                                        $option->selected(explode(',', $newscategoriesIds));
                                    })
                                    ->multiple()
                                    ->searchable()
                                    ->helperText(__('Leave newscategories empty if you want to show newsposts from all newscategories.'))
                                    ->toArray()
                            );
                    }
                );
        ...
        if (defined('THEME_FRONT_HEADER') && setting('news_post_schema_enabled', 1)) {
            add_action(BASE_ACTION_PUBLIC_RENDER_SINGLE, function ($screen, $post) {
                add_filter(THEME_FRONT_HEADER, function ($html) use ($post) {
                    ...
                    $schemaType = setting('news_post_schema_type', 'NewsArticle');

                    if (! in_array($schemaType, ['NewsArticle', 'News', 'Article', 'NewsPosting'])) {
                    ...
                    return $html . Html::tag('script', json_encode($schema), ['type' => 'news/ld+json'])
                            ->toHtml();
                }, 35);
            }, 35, 2);
        }
    ...
    public function addThemeOptions(): void
    {
        ...
        theme_option()
            ->setSection([
                'title' => trans('plugins/news::base.settings.title'),
                ...
                'fields' => [
                    [
                        ...
                        'attributes' => [
                            'name' => 'news_page_id',
                            'list' => [0 => trans('plugins/news::base.select')] + $pages,
                            'value' => '',
                            'options' => [
                                'class' => 'form-control',
                            ],
                    ...
                    [
                        'id' => 'number_of_newsposts_in_a_category',
                        'type' => 'number',
                        'label' => trans('plugins/news::base.number_newsposts_per_page_in_category'),
                        'attributes' => [
                            'name' => 'number_of_newsposts_in_a_category',
                            'value' => 12,
                            'options' => [
                                'class' => 'form-control',
                            ],
                    ...
                    [
                        'id' => 'number_of_newsposts_in_a_tag',
                        'type' => 'number',
                        'label' => trans('plugins/news::base.number_newsposts_per_page_in_tag'),
                        'attributes' => [
                            'name' => 'number_of_newsposts_in_a_tag',
                            'value' => 12,
                            'options' => [
                                'class' => 'form-control',
                            ],
    ...
    {
        if (Auth::guard()->user()->hasPermission('newscategories.index')) {
            Menu::registerMenuOptions(Category::class, trans('plugins/news::categories.menu'));
        }

        if (Auth::guard()->user()->hasPermission('newstags.index')) {
    ...
    {
        if (! Auth::guard()->user()->hasPermission('newsposts.index')) {
            return $widgets;
        }
        ...
        return (new DashboardWidgetInstance())
            ->setPermission('newsposts.index')
            ->setKey('widget_newsposts_recent')
            ->setTitle(trans('plugins/news::posts.widget_newsposts_recent'))
            ->setIcon('fas fa-edit')
            ->setColor('yellow')
            ->setRoute(route('newsposts.widget.recent-newsposts'))
            ->setBodyClass('')
    ...
    public function renderNewsnewsposts(Shortcode $shortcode): array|string
    {
        ...
        $newsposts = Post::query()
            ...
            ->when(! empty($categoryIds), function ($query) use ($categoryIds) {
                $query->whereHas('newscategories', function ($query) use ($categoryIds) {
                    $query->whereIn('newscategories.id', $categoryIds);
                });
            })
            ->paginate((int)$shortcode->paginate ?: 12);

        $view = 'plugins/news::themes.templates.newsposts';
        $themeView = Theme::getThemeNamespace() . '::views.templates.newsposts';

        if (view()->exists($themeView)) {
            $view = $themeView;
        }

        return view($view, compact('newsposts'))->render();
    }

    public function renderNewsPage(string|null $content, Page $page): string|null
    {
        if ($page->getKey() == $this->getNewsPageId()) {
            ...
            return view($view, [
                'newsposts' => get_all_newsposts(true, (int)theme_option('number_of_newsposts_in_a_category', 12)),
            ])->render();
        }
    ...
    protected function getNewsPageId(): int|string|null
    {
