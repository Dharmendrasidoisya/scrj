    ...
    public function handleFrontRoutes(Slug|array $slug): Slug|array|Builder
    {
        ...
        switch ($slug->reference_type) {
            case Post::class:
                ...
                $post = Post::query()
                    ->where($condition)
                    ->with(['industrycategories', 'industrytags', 'slugable', 'industrycategories.slugable', 'industrytags.slugable'])
                    ->firstOrFail();
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/industry::posts.edit_this_post'),
                        route('industryposts.edit', $post->getKey()),
                        null,
                        'industryposts.edit'
                    );
                }
                ...
                if (function_exists('shortcode')) {
                    shortcode()->getCompiler()->setEditLink(route('industryposts.edit', $post->id), 'industryposts.edit');
                }

                $category = $post->industrycategories->sortByDesc('id')->first();
                if ($category) {
                ...
                return [
                    'view' => 'post',
                    'default_view' => 'plugins/industry::themes.post',
                    'data' => compact('post'),
                    'slug' => $post->slug,
                ];
            case Category::class:
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/industry::categories.edit_this_category'),
                        route('industrycategories.edit', $category->getKey()),
                        null,
                        'industrycategories.edit'
                    );
                }

                $allRelatedCategoryIds = array_merge([$category->getKey()], $category->activeChildren->pluck('id')->all());

                $industryposts = app(PostInterface::class)
                    ->getByCategory($allRelatedCategoryIds, (int)theme_option('number_of_industryposts_in_a_category', 12));
                ...
                return [
                    'view' => 'category',
                    'default_view' => 'plugins/industry::themes.category',
                    'data' => compact('category', 'industryposts'),
                    'slug' => $category->slug,
                ];
            case Tag::class:
                $stag = Tag::query()
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/industry::tags.edit_this_tag'),
                        route('industrytags.edit', $stag->getKey()),
                        null,
                        'industrytags.edit'
                    );
                }

                $industryposts = get_industryposts_by_tag($stag->getKey(), (int)theme_option('number_of_industryposts_in_a_tag', 12));
                ...
                return [
                    'view' => 'stag',
                    'default_view' => 'plugins/industry::themes.stag',
                    'data' => compact('stag', 'industryposts'),
                    'slug' => $stag->slug,
                ];
        }
