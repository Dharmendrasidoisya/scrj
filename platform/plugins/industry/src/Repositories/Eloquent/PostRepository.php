...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('industrycategories', function (Builder $query) use ($id) {
                $query->whereIn('industrycategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getRelatedCategoryIds(Post|int|string $model): array
    {
        ...
        try {
            return $model->industrycategories()->allRelatedIds()->toArray();
        } catch (Exception) {
            return [];
        }
    }
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('industrycategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('industrycategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    {
        $data = $this->model
            ->with(['slugable', 'industrycategories', 'industrycategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('industrytags', function (Builder $query) use ($stag) {
                $query->where('industrytags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentindustryposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('industrycategories', function (Builder $query) use ($categoryId) {
                    $query->where('industrycategories.id', $categoryId);
                });
        }
    ...
    public function getAllindustryposts(
        int $perPage = 12,
        bool $active = true,
        array $with = ['slugable']
    ): Collection|LengthAwarePaginator {
    ...
    public function getPopularindustryposts(int $limit, array $args = []): Collection
    {
        $data = $this->model
            ->with('slugable')
            ->orderByDesc('views')
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['industrycategories'] !== null) {
            $industrycategories = array_filter((array)$filters['industrycategories']);

            $data = $data->whereHas('industrycategories', function (Builder $query) use ($industrycategories) {
                $query->whereIn('industrycategories.id', $industrycategories);
            });
        }

        if ($filters['industrycategories_exclude'] !== null) {
            $data = $data
                ->whereHas('industrycategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('industrycategories.id', array_filter((array)$filters['industrycategories_exclude']));
                });
        }
